...
class PillarFeatureNet(nn.Module):
    ...
    def forward(self, features: Tensor, num_points: Tensor, coors: Tensor,
        ...
        # print(f"point_cloud_range 的类型: {type(self.point_cloud_range)}")
        # print(f"grid 的类型: {type(self.grid_size)}")
        # print(f"grip 的形状: {self.grid_size}")
        ...
        # print(f"PillarFeatureNet Feature type: {features.dtype}, Shape: {features.shape}")


        return features.squeeze(1)

@MODELS.register_module()
class PillarFeatureNetV2(nn.Module):
    """Pillar Feature Net.
    ...
    def __init__(self,
                 ...
                 mode: Optional[str] = 'max',
                 legacy: Optional[bool] = True):
        super(PillarFeatureNetV2, self).__init__()
        assert len(feat_channels) > 0
        self.legacy = legacy
        if with_cluster_center:
            in_channels += 3
    ...
    def forward(self, features: Tensor, num_points: Tensor, coors: Tensor,
        ...
        voxel_features_aggregated = features.squeeze(1)  # 形狀 (N, C)

        # print(voxel_features_aggregated.shape)
        # print(coors.int().shape)
        # print([self.vy, self.vx])
        # print(kwargs.get('batch_size', 1))

        sparse_tensor = SparseConvTensor(
            features=voxel_features_aggregated,  # 特徵
            indices=coors.int(),  # 索引
            spatial_shape=[self.vy, self.vx],  # 空間形狀
            batch_size=kwargs.get('batch_size')  # 批次大小
        )


        # print(f"XXXXXXXXXXXXXOutput shapeXXXXXX: {sparse_tensor.shape}")
